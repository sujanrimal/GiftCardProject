...
public class ConnectionPoolTest extends HybrisJUnit4Test
{
	...
	public void testMultithreadedAccess() //NOPMD
	{
		...
		doTestMultithreadedAccess(RUN_SECONDS, THREADS, 80, 0, sendDummyStatement);

		// 100% non-tx connections
		doTestMultithreadedAccess(RUN_SECONDS, THREADS, 100, 0, sendDummyStatement);

		// 50% non tx 50% tx connections - no rollbacks
		doTestMultithreadedAccess(RUN_SECONDS, THREADS, 50, 0, sendDummyStatement);

		// 80% non tx 20 % tx connections 1% rollbacks (this may break MySQL!)
		doTestMultithreadedAccess(RUN_SECONDS, THREADS, 80, 2, sendDummyStatement);
	}

	private void doTestMultithreadedAccess(final int RUN_SECONDS, final int THREADS, final int PERCENT_NO_TX,
	                                       final int PERCENT_TX_ROLLBACK,
	                                       final boolean sendDummyStatement)
	{
		...
		LOG.info("--- test multithreaded access to connection pool duration:" + RUN_SECONDS + "s threads:" + THREADS + " nonTx:"
				         + PERCENT_NO_TX + "% rollback:" + PERCENT_TX_ROLLBACK + "% "
				         + "-----------------------------------");
		try
		{
			final Collection<TestConnectionImpl> allConnections = new ConcurrentLinkedQueue<>();
			...
			final TestThreadsHolder threadsHolder = new TestThreadsHolder(THREADS, runnable)
			{
				...
				public void stopAll()
				{
					ConnectionPoolTest.LOG.info("stoping all threads");
			...
			assertTrue(
					"handed out more than max connections (got:" + connectionCounter.get() + " > max:" + maxAllowedConnections + ")",
					//
					connectionCounter.get() <= maxAllowedConnections);
			assertEquals("still got " + stats.borrowed + "borrowed connections", 0, stats.borrowed);
			assertEquals(
					"connection count mismatch - total:" + connectionCounter.get() + " <> " + (stats.returned + stats.invalidated)
							+ " (returned:" + stats.returned + " + invalidated:" + stats.invalidated + ")", //
			...
			assertTrue(
					"data source " + dataSource + " had more than max allowed connections (max:" + maxConnections + ", max in use:"
							+ dataSource.getMaxInUse() + ")", //
					maxConnections >= dataSource.getMaxInUse());
			assertTrue(
					"data source " + dataSource + " had more than max allowed physical connections (max:" + maxConnections
	...
	private void stopThreads(final TestThreadsHolder threadsHolder)
	{
		if (threadsHolder != null)
		{
			try
			{
				threadsHolder.stopAndDestroy(10);
			}
			catch (final Exception e)
			{
				// ignore silently
	...
	private static class ContinuousAccessRunnable implements Runnable
	{
		...
		public ContinuousAccessRunnable(final HybrisDataSource dataSource, final int percentNoTX, final int percentTxRollback,
		                                final AtomicLong rollbackCounter, final AtomicBoolean finished, final String runId,
		                                final boolean sendStatement)
		{
	...
	private static class GetOneConnectionRunnable implements Runnable
	{
		...
		public void run()
		{
			...
			{
				ConnectionPoolTest.LOG.error("runner has been interrupted");
			}
			catch (final Exception e)
			{
				ConnectionPoolTest.LOG.error(e.getMessage(), e);
			}
			finally
			{
				JdbcUtils.closeConnection(connection);
	...
	private static class TestJDBCConnectionPool extends JDBCConnectionPool
	{
		...
		public TestJDBCConnectionPool(final JDBCConnectionFactory factory, final GenericObjectPoolConfig cfg)
		{
			super(factory, cfg);
		}
	...
	private static class TestConnectionImpl extends ConnectionImpl
	{
		...
		public synchronized void assertNotBorrowed(final String caller)
		{
			if (borrowingThread != null)
			{
				ConnectionPoolTest.LOG.error(
						"connection " + this + " is borrowed by " + borrowingThread + ", caller is " + caller + "/"
		...
		public synchronized void assertBorrowed(final String caller)
		{
			if (borrowingThread == null)
			{
				ConnectionPoolTest.LOG
						.error("connection " + this + " is not borrowed, caller is " + caller + "/" + Thread.currentThread()
						                                                                                    .getName());
			}
		}
		...
		public synchronized void assertNotReturned(final String caller)
		{
			...
			{
				ConnectionPoolTest.LOG.error(
						"connection " + this + " is returned by " + returningThread + ", caller is " + caller + "/"
								+ Thread.currentThread().getName());
			}
		}
		...
		public synchronized void assertReturned(final String caller)
		{
			...
			{
				ConnectionPoolTest.LOG
						.error("connection " + this + " is not returned, caller is " + caller + "/" + Thread.currentThread()
						                                                                                    .getName());
			}
		}
		...
		public synchronized void assertNotInvalidated(final String caller)
		{
			...
			{
				ConnectionPoolTest.LOG
						.error("connection " + this + " is already invalidated by " + invalidatingThread + ", caller is " + caller + "/"
								       + Thread.currentThread().getName());
			}
		}
		...
		public synchronized void assertInvalidated(final String caller)
		{
			...
			{
				ConnectionPoolTest.LOG
						.error("connection " + this + " is not invalidated, caller is " + caller + "/" + Thread.currentThread()
						                                                                                       .getName());
			}
		}
		...
		public synchronized void assertClosedForReal(final String caller)
		{
			...
			{
				ConnectionPoolTest.LOG.error(
						"connection " + this + " is not closed for real, caller is " + caller + "/" + Thread.currentThread()
						                                                                                    .getName());
			}
		}
		...
		public synchronized void assertNotClosedForReal(final String caller)
		{
			...
			{
				ConnectionPoolTest.LOG
						.error("connection " + this + " is already closed by " + realClosingThread + ", caller is " + caller + "/"
								       + Thread.currentThread().getName());
			}
		}
		...
		public synchronized void markBorrowed(final String caller)
		{
			...
			if (logToConsole)
			{
				ConnectionPoolTest.LOG.info("borrowed " + this + " caller is " + caller + "/" + borrowingThread);
			}
		}
		...
		public synchronized void markReturned(final String caller)
		{
			...
			if (logToConsole)
			{
				ConnectionPoolTest.LOG.info("returned " + this + " caller is " + caller + "/" + returningThread);
			}
		}
		...
		public synchronized void markInvalidated(final String caller)
		{
			...
			if (logToConsole)
			{
				ConnectionPoolTest.LOG.info("invalidated " + this + " caller is " + caller + "/" + invalidatingThread);
			}
		}
		...
		public synchronized void markClosedForReal(final String caller)
		{
			...
			if (logToConsole)
			{
				ConnectionPoolTest.LOG.info("closed for real " + this + " caller is " + caller + "/" + realClosingThread);
			}
		}
