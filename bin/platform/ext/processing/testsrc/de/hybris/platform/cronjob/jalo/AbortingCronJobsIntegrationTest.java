...
public class AbortingCronJobsIntegrationTest extends ServicelayerBaseTest
{
	...
	private final AbstractEventListener<AfterCronJobCrashAbortEvent> listener = new AbstractEventListener<AfterCronJobCrashAbortEvent>()
	{
		@Override
	...
	public void shouldAbortCronJobOnRunningOnGivenNode()
	{
		...
		testCronJobManager.abortRunningCronJobsForClusterNodes(Collections.unmodifiableSet(new HashSet(Arrays.asList(currentNodeId + 1))), 100);
	...
	public void shouldNotAbortCronJobsInStatusesOtherThanRunning()
	{
		...
		final Map<PK, Long> versions = Stream.of(cjAborted, cjFinished, cjPaused, cjUnknown).peek(i -> modelService.detach(i))
				.collect(Collectors.toMap(CronJobModel::getPk, i -> i.getItemModelContext().getPersistenceVersion()));


		testCronJobManager.abortRunningCronJobsForClusterNodes(Collections.unmodifiableSet(new HashSet(Arrays.asList(currentNodeId))), 100);
