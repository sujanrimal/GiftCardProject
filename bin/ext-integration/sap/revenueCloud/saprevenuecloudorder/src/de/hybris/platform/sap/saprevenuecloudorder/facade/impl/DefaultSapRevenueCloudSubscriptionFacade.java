...
public class DefaultSapRevenueCloudSubscriptionFacade extends DefaultSubscriptionFacade
	...
	protected static DateTimeFormatter utcDateFormatter = DateTimeFormatter.ofPattern(SaprevenuecloudorderConstants.DATE_TIME_PATTERN);
	...
	public SubscriptionData getSubscription(final String subscriptionId) throws SubscriptionFacadeException 
	{
		...
		SubscriptionData subscriptionData = sapSubscriptionDetailConverter.convert(sapSubscription);

		//Set Date in ISO format
		subscriptionData.setValidTillDate(sapSubscription.getValidUntil());
	...
	protected String calculateExtensionDate(final String billingPlanId, final String validTillDate,
			final String extensionPeriod)
	{
		...
		try 
		{
			...
			ZonedDateTime zonedDateTime = ZonedDateTime.parse(validTillDate, DateTimeFormatter.ISO_ZONED_DATE_TIME);
			String timeZone = getConfigurationService().getConfiguration().getString(SaprevenuecloudorderConstants.DEFAULT_TIMEZONE);
			ZonedDateTime localZonedDateTime = zonedDateTime.toLocalDate().atStartOfDay(ZoneId.of(timeZone));
			localZonedDateTime = localZonedDateTime.plusMonths(noOfMonths);
			extensionDate = localZonedDateTime
					.withZoneSameInstant(ZoneId.of("UTC"))
					.format(utcDateFormatter);
		...
		{
			LOG.warn("Exception while parsing dates :" + e);
		}
