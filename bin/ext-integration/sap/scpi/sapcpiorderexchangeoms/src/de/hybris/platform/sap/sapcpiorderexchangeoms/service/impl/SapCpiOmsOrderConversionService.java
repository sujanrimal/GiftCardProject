...
public class SapCpiOmsOrderConversionService extends SapCpiOmmOrderConversionService {
    ...
    public SapCpiOrder convertOrderToSapCpiOrder(OrderModel orderModel) {
        ...
        if (LOG.isDebugEnabled()) {
            LOG.debug(String.format("SCPI OMS order object: %n %s", ReflectionToStringBuilder.toString(sapCpiOmsOrder, ToStringStyle.DEFAULT_STYLE)));
        }
    ...
    public List<SapCpiOrderCancellation> convertCancelOrderToSapCpiCancelOrder(OrderCancelRecordEntryModel orderCancelRecordEntryModel) {
        ...
        getSapOrderCancelRequestContributor().createRows(orderCancelRecordEntryModel)
                ...
                rowSet -> {
                    ...
                    if (LOG.isDebugEnabled()) {
                        LOG.debug(String.format("SCPI cancel order object: %n %s", ReflectionToStringBuilder.toString(sapCpiOrderCancellation, ToStringStyle.DEFAULT_STYLE)));
                    }
                }
        );
    ...
    protected SapCpiConfig mapOrderCancellationConfigInfo(String sapLogicalSystemName) {
        ...
        SapCpiConfig sapCpiConfig = new SapCpiConfig();
        sapCpiConfig.setSapCpiTargetSystem(mapTargetSystem(readSapLogicalSystem(sapLogicalSystemName)));
    ...
    protected SapCpiTargetSystem mapTargetSystem(SAPLogicalSystemModel sapLogicalSystem) {
        ...
        if (sapLogicalSystem.getSapHTTPDestination() != null) {
            ...
            String[] sapClientSplit = targetUrl.split("sap-client=");

            if (sapClientSplit.length == 2 && StringUtils.isNotBlank(sapClientSplit[1])) {
                sapCpiTargetSystem.setClient(sapClientSplit[1].substring(0, 3));
            }else{
                sapCpiTargetSystem.setClient(StringUtils.EMPTY);
            }
