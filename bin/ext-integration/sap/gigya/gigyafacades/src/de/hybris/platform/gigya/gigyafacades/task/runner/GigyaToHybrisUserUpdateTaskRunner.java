...
public class GigyaToHybrisUserUpdateTaskRunner implements TaskRunner<TaskModel>
{
	 ...
	 * direction.
	 */
	private Map<GigyaConfigModel, List<GigyaFieldMappingModel>> collectRequiredMappings(String apiKey) {
		...
		if (CollectionUtils.isNotEmpty(allMappings)) {
			allMappings.forEach(item -> {
				if (isSyncDirectionFromG2H(item.getSyncDirection()) && !item.isCustom()
						&& isValidForApiKey(apiKey, item.getGigyaConfig())) {
