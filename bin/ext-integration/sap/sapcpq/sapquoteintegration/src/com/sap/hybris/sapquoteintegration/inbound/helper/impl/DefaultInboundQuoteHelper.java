...
public class DefaultInboundQuoteHelper implements InboundQuoteHelper {
	...
	protected void saveProposalDocument(QuoteModel inboundQuote) {
		...
		if (externalProposalDocument != null && !externalProposalDocument.isEmpty()) {
			...
			final CatalogModel cm = catalogService.getCatalogForId(getConfigurationService().getConfiguration()
	                .getString(SapquoteintegrationConstants.CATALOG_FOR_PROPOSAL_DOCUMENTS));
			...
			mediaService.setStreamForMedia(uploadFileMedia, inputStream, randomFileName, "application/html");
		}
