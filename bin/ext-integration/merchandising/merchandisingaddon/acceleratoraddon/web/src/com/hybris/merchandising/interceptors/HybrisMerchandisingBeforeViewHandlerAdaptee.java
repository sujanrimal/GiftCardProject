...
public class HybrisMerchandisingBeforeViewHandlerAdaptee implements BeforeViewHandlerAdaptee
{
	 ...
	 */
	private void storeTenant(final Session currentSession)
	{
	...
	private void storeContextualData(final String propertyName, final Object propertyValue)
	{
	...
	protected void storeFacets(final Session currentSession, final ModelMap model)
	{
		Optional.ofNullable(model.get(SEARCH_PAGE_DATA))
				...
				.map(searchPageData -> searchPageData.getBreadcrumbs())
				.ifPresent(breadcrumbs -> {
					if (!breadcrumbs.isEmpty())
					{
						final List<Facet> values = new ArrayList<>(getFacetBreadcrumbs(breadcrumbs).values());
	...
	{
		final Map<String, Facet> selectedFacets = new HashMap<String, Facet>();
	...
	protected void populateJSAddOnVariables(final ModelMap model)
	{
		...
		Optional.ofNullable(sessionService.getAttribute(MerchandisingConstants.CONTEXT_STORE_KEY))
				...
				.ifPresent(contextRepository -> retrieveContextualDataFromActions(contextRepository, new ObjectMapper(),
