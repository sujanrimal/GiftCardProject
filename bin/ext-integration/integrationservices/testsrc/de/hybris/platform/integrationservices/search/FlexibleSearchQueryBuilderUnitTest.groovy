...
class FlexibleSearchQueryBuilderUnitTest extends Specification {
	...
	def "generates SELECT FROM #fromClause type for an existing integration object item of #type type"() {
		...
		'composed'    | [itemWithoutKey('myItem'), itemWithoutKey('anotherItem')] | '{myItem* AS myitem}'
		'enumeration' | [itemWithoutKey('other'), enumerationItem('myItem')]      | '{myItem AS myitem}'
		'abstract'    | [itemWithoutKey('other'), abstractItem('myItem')]         | '{myItem AS myitem}'
	}
	...
	def "WHERE clause is not generated if the key condition added for an item with reference key"() {
		...
		def itemMetadata = itemModelBuilder()
				.withUniqueAttribute(simpleAttributeBuilder()
						.withName("code")
						.withIntegrationObjectItemCode("ReferencedType")
						.unique())
				.build()
	...
	def itemWithoutKey(final String code) {
		itemWithoutKey(code, code)
	}

	def itemWithoutKey(final String integrationCode, final String platformCode) {
		def item = item(integrationCode, platformCode)
	...
	private IntegrationObjectItemModel itemWithSimpleKey(String attrName, String qualifier) {
		def item = item('myObject', "Product")
