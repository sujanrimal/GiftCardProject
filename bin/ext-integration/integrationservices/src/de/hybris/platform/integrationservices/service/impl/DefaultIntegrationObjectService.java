...
public class DefaultIntegrationObjectService implements IntegrationObjectService
{
	...
	@Override
	public IntegrationObjectItemModel findIntegrationObjectItemByParentTypeCode(final String integrationObjectCode, final String typeCode)
	{
		final String queryString =
				"SELECT DISTINCT {inteObjItem." + IntegrationObjectItemModel.PK + "}"
						+ " FROM {" + IntegrationObjectItemModel._TYPECODE + " as inteObjItem"
		...
		return getIntegrationObjectItemModel(integrationObjectCode, queryString, params);
	}

	protected IntegrationObjectItemModel getIntegrationObjectItemModel(final String integrationObjectCode, final String queryString, final Map<String, Object> params)
	{
		final SearchResult<IntegrationObjectItemModel> search = getFlexibleSearchService().search(queryString, params);
		final List<IntegrationObjectItemModel> list = search.getResult();
		if (list.isEmpty())
	...
	protected List<PK> getSuperTypePks(final String typeCode)
	{
		final ComposedTypeModel itemType = getTypeService().getComposedTypeForCode(typeCode);
