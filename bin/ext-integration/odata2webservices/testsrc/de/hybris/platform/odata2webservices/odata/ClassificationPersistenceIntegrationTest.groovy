...
class ClassificationPersistenceIntegrationTest extends ServicelayerTransactionalSpockSpecification {
	...
	@Test
	@Unroll
	def "product #condition classification is created"() {
		given:
		importProductFeatureIO()
		...
		when:
		def postResponse = postProduct(PRODUCT_IO, content)
		...
		featureIntegrationKeyAt(0, getResponse) == featureIntegrationKey

		where:
		condition 				| content								| featureIntegrationKey
		'feature without unit'	| productFeatureWithoutUnitContent()	| "$CATALOG_VERSION_VERSION|$CATALOG_ID|$PRODUCT_FEATURE_QUALIFIER|$PRODUCT_FEATURE_VALUE_POSITION|$PRODUCT_CODE"
		'feature with unit'		| productFeatureWithUnitContent()       | "$CATALOG_VERSION_VERSION|${ClassificationSystemBuilder.ID}|$CATALOG_ID|${ClassificationAttributeUnitBuilder.CODE}|${ClassificationSystemVersionBuilder.VERSION}|$PRODUCT_FEATURE_QUALIFIER|$PRODUCT_FEATURE_VALUE_POSITION|$PRODUCT_CODE"
	}
	...
	private static void importProductFeatureIO() {
		importImpEx(
	...
	private static String productFeatureWithoutUnitContent() {
		product()
				.withField('features', [feature()])
				.build()
	}
	...
	private ODataResponse getProductFeatures(String service, String integrationKey) {
		...
		facade.handleGetEntity(createContext(getRequest))
	}
