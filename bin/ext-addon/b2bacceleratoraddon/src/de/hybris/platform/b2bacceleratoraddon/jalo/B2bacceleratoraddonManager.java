 ...
 */
public class B2bacceleratoraddonManager extends GeneratedB2bacceleratoraddonManager
{
	/** Edit the local|project.properties to change logging behavior (properties 'log4j.*'). */
	private static final Logger LOG = Logger.getLogger(B2bacceleratoraddonManager.class.getName());
	 ...
	 */
	public static B2bacceleratoraddonManager getInstance()
	{
		return (B2bacceleratoraddonManager) Registry.getCurrentTenant().getJaloConnection().getExtensionManager().getExtension(
				B2bacceleratoraddonConstants.EXTENSIONNAME);
	}
	 ...
	 */
	public B2bacceleratoraddonManager() 
	{
		if (LOG.isDebugEnabled())
		{
			LOG.debug("constructor of B2bacceleratoraddonManager called.");
		}
	}
	...
	public void init()
	{
		if (LOG.isDebugEnabled())
		{
			LOG.debug("init() of B2bacceleratoraddonManager called. " + getTenant().getTenantID());
		}
	}
	...
	public void destroy()
	{
		if (LOG.isDebugEnabled())
		{
			LOG.debug("destroy() of B2bacceleratoraddonManager called, current tenant: " + getTenant().getTenantID());
		}
	}
