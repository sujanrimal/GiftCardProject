 ...
 */
public class PcmbackofficesamplesaddonManager extends GeneratedPcmbackofficesamplesaddonManager
{
	/** Edit the local|project.properties to change logging behavior (properties 'log4j.*'). */
	private static final Logger LOG = Logger.getLogger(PcmbackofficesamplesaddonManager.class.getName());
	 ...
	 */
	public static PcmbackofficesamplesaddonManager getInstance()
	{
		return (PcmbackofficesamplesaddonManager) Registry.getCurrentTenant().getJaloConnection().getExtensionManager().getExtension(
				PcmbackofficesamplesaddonConstants.EXTENSIONNAME);
	}
	 ...
	 */
	public PcmbackofficesamplesaddonManager() // NOPMD 
	{
		if (LOG.isDebugEnabled())
		{
			LOG.debug("constructor of PcmbackofficesamplesaddonManager called.");
		}
	}
	...
	public void init()
	{
		if (LOG.isDebugEnabled())
		{
			LOG.debug("init() of PcmbackofficesamplesaddonManager called. " + getTenant().getTenantID());
		}
	}
	...
	public void destroy()
	{
		if (LOG.isDebugEnabled())
		{
			LOG.debug("destroy() of PcmbackofficesamplesaddonManager called, current tenant: " + getTenant().getTenantID());
		}
	}
