 ...
 */
public class CommerceorgaddonManager extends GeneratedCommerceorgaddonManager
{
	/** Edit the local|project.properties to change logging behavior (properties 'log4j.*'). */
	private static final Logger LOG = Logger.getLogger(CommerceorgaddonManager.class.getName());
	 ...
	 */
	public static CommerceorgaddonManager getInstance()
	{
		return (CommerceorgaddonManager) Registry.getCurrentTenant().getJaloConnection().getExtensionManager().getExtension(
				CommerceorgaddonConstants.EXTENSIONNAME);
	}
	 ...
	 */
	public CommerceorgaddonManager() 
	{
		if (LOG.isDebugEnabled())
		{
			LOG.debug("constructor of CommerceorgaddonManager called.");
		}
	}
	...
	public void init()
	{
		if (LOG.isDebugEnabled())
		{
			LOG.debug("init() of CommerceorgaddonManager called. " + getTenant().getTenantID());
		}
	}
	...
	public void destroy()
	{
		if (LOG.isDebugEnabled())
		{
			LOG.debug("destroy() of CommerceorgaddonManager called, current tenant: " + getTenant().getTenantID());
		}
	}
