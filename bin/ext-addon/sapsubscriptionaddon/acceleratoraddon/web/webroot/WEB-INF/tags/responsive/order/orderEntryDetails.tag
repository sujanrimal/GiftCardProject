...
<c:if test="${not empty orderEntry.product.subscriptionTerm}">
	<c:set var="varShowStock" value="false" />	
</c:if>
    ...
    <div class="item__info">
        ...
        <c:choose>
			<c:when test="${not empty orderEntry.product.subscriptionTerm}">
				<c:forEach items="${orderEntry.orderEntryPrices}"
					var="orderEntryPrice">
					<!-- please ensure that these TDs get rendered always to avoid a uneven number of TDs per Row -->
					<c:if
						test='${orderEntryPrice.billingTime ne null and orderEntryPrice.billingTime.code ne "paynow"}'>
						<div class="price">
							<format:price priceData="${orderEntryPrice.basePrice}" displayFreeForZero="false" />
							&nbsp;${orderEntryPrice.billingTime.name}
						</div>
        ...
        <%-- availability --%>
        <c:if test="${empty orderEntry.product.subscriptionTerm}">
