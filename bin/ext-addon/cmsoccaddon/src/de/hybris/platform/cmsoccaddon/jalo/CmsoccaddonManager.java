...
import de.hybris.platform.cmsoccaddon.jalo.GeneratedCmsoccaddonManager;
 ...
 */
public class CmsoccaddonManager extends GeneratedCmsoccaddonManager
{
	/** Edit the local|project.properties to change logging behavior (properties 'log4j.*'). */
	private static final Logger LOG = Logger.getLogger(CmsoccaddonManager.class.getName());
	 ...
	 */
	public CmsoccaddonManager() // NOPMD
	{
		if (LOG.isDebugEnabled())
		{
			LOG.debug("constructor of CmsoccaddonManager called.");
		}
	}
	 ...
	 */
	public static CmsoccaddonManager getInstance()
	{
		return (CmsoccaddonManager) Registry.getCurrentTenant().getJaloConnection().getExtensionManager()
				.getExtension(CmsoccaddonConstants.EXTENSIONNAME);
	}
	...
	public void init()
	{
		if (LOG.isDebugEnabled())
		{
			LOG.debug("init() of CmsoccaddonManager called. " + getTenant().getTenantID());
		}
	}
	...
	public void destroy()
	{
		if (LOG.isDebugEnabled())
		{
			LOG.debug("destroy() of CmsoccaddonManager called, current tenant: " + getTenant().getTenantID());
		}
	}
