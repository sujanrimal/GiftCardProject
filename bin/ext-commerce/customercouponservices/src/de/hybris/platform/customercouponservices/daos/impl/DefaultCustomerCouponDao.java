...
public class DefaultCustomerCouponDao extends DefaultGenericDao<CustomerCouponModel> implements CustomerCouponDao
{
	...
	private static final String SEARCH_COUPON_QUERY = "select {cp:pk} from {" + CustomerCouponModel._TYPECODE + " as cp left join "
			...
			+ " as cst on {cst:pk} = {ccr:target}} where {" + CustomerCouponModel.ACTIVE + "} = ?" + COUPON_ACTIVE + " and {"
			+ CustomerCouponModel.ENDDATE + "} > ?now and {cst:" + CustomerModel.PK + "} = ?customer ";
	...
	private static final String SEARCH_EFFECTIVE_COUPON_QUERY = "select {cp.pk} from {" + CustomerCouponModel._TYPECODE
			...
			+ CustomerModel._TYPECODE + " as cst on {cst.pk} = {ccr:target} } where {" + CustomerCouponModel.ACTIVE + "} = ?"
			+ COUPON_ACTIVE + " and {" + CustomerCouponModel.ENDDATE + "} > ?now and {" + CustomerCouponModel.STARTDATE
			+ " } < ?now and {cst." + CustomerModel.UID + "} = ?customerUid";

	private static final String CHECK_EFFECTIVE_COUPON_QUERY = "select {cp.pk} from {" + CustomerCouponModel._TYPECODE
			+ " as cp left join " + CUSTOMERCOUPON_CUSTOMER_RELATION + " as ccr on {ccr:source} = {cp:pk} left join "
			+ CustomerModel._TYPECODE + " as cst on {cst.pk} = {ccr:target} } where {" + CustomerCouponModel.ACTIVE + "} = ?"
			+ COUPON_ACTIVE + " and {" + CustomerCouponModel.ENDDATE + "} > ?now and {" + CustomerCouponModel.STARTDATE
			+ " } < ?now and {cst." + CustomerModel.UID + "} = ?customerUid and {" + CustomerCouponModel.COUPONID
			+ " } = ?couponCode";

	private static final String CHECK_ASSIGNED_COUPON_QUERY = "select {cp.pk} from {" + CustomerCouponModel._TYPECODE
			+ " as cp left join " + CUSTOMERCOUPON_CUSTOMER_RELATION + " as ccr on {ccr:source} = {cp:pk} left join "
			+ CustomerModel._TYPECODE + " as cst on {cst.pk} = {ccr:target} } where {" + CustomerCouponModel.ACTIVE + "} = ?"
			+ COUPON_ACTIVE + " and {cst." + CustomerModel.UID + "} = ?customerUid and {" + CustomerCouponModel.COUPONID
			+ " } = ?couponCode";
	...
	private static final String FIND_ASSIGNABLE_COUPONS_QUERY = "SELECT {C:" + CustomerCouponModel.PK + "} FROM {"
			+ CustomerCouponModel._TYPECODE + " AS C} WHERE {C:" + CustomerCouponModel.ASSIGNABLE + "} = ?"
			+ CUSTOMER_COUPON_ASSIGNABLE + " AND {C:" + CustomerCouponModel.STARTDATE + "} IS NOT NULL AND {C:"
			+ CustomerCouponModel.ENDDATE + "} IS NOT NULL AND {C:" + CustomerCouponModel.ENDDATE + "} >= ?now AND {C:"
			+ CustomerCouponModel.ACTIVE + "} = ?" + COUPON_ACTIVE + " AND {C:" + CustomerCouponModel.PK
			+ "} NOT IN ({{SELECT {CC:pk} FROM {" + CustomerCouponModel._TYPECODE
			...
			final CustomerModel customer, final PageableData pageableData)
	{
		...
		params.put(COUPON_ACTIVE, Boolean.TRUE);
	...
	{
		final FlexibleSearchQuery query = new FlexibleSearchQuery(CHECK_ASSIGNED_COUPON_QUERY);
		query.addQueryParameter(COUPON_ACTIVE, Boolean.TRUE);
		query.addQueryParameter(CUSTOMERUID, customer.getUid());
		query.addQueryParameter("couponCode", couponCode);
	...
	{
		return findAssignmentCoupons(customer, text, FIND_ASSIGNABLE_COUPONS_QUERY, buildParamsMapForAssignableCoupons(customer));
	}
	...
	protected Map<String, Object> buildParamsMapForAssignableCoupons(final CustomerModel customer)
	{
		final Map<String, Object> params = new HashMap<>(0);
		params.put(CUSTOMER_COUPON_ASSIGNABLE, Boolean.TRUE);
		params.put(COUPON_ACTIVE, Boolean.TRUE);
	...
	protected Map<String, Object> buildQueryParamsForAssignedCoupons(final CustomerModel customer)
	{
		final Map<String, Object> params = new HashMap<>(0);
		params.put("now", Calendar.getInstance().getTime());
		params.put(CUSTOMER, customer);
