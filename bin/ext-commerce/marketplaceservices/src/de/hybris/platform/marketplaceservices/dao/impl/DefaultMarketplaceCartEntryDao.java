...
public class DefaultMarketplaceCartEntryDao extends DefaultCartEntryDao implements MarketplaceCartEntryDao
{
	...
	protected static final String FIND_UNSALEABLE_CARTENTRIES_IN_CART = "select {A:" + CartEntryModel.PK + "} from {"
			...
			+ "}} WHERE {B:" + ProductModel.SALEABLE + "} = ?" + PRODUCT_SALEABLE + " and {C:" + CartModel.CODE + "} = ?"
			+ CART_CODE;
	...
	public List<CartEntryModel> findUnSaleableCartEntries(final CartModel cart)
	{
		...
		final FlexibleSearchQuery query = new FlexibleSearchQuery(FIND_UNSALEABLE_CARTENTRIES_IN_CART);
		query.addQueryParameter(PRODUCT_SALEABLE, Boolean.FALSE);
