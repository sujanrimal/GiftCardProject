...
@SuppressWarnings(
{ "deprecation" })
public class CommercewebservicescommonsManager extends GeneratedCommercewebservicescommonsManager
{
	/** Edit the local|project.properties to change logging behavior (properties 'log4j.*'). */
	private static final Logger LOG = Logger.getLogger(CommercewebservicescommonsManager.class.getName());
	 ...
	 */
	public CommercewebservicescommonsManager()
	{
		if (LOG.isDebugEnabled())
		{
			LOG.debug("constructor of CommercewebservicescommonsManager called.");
		}
	}
	 ...
	 */
	public static CommercewebservicescommonsManager getInstance()
	{
		return (CommercewebservicescommonsManager) Registry.getCurrentTenant().getJaloConnection().getExtensionManager()
				.getExtension(CommercewebservicescommonsConstants.EXTENSIONNAME);
	}
	...
	public void init()
	{
		if (LOG.isDebugEnabled())
		{
			LOG.debug("init() of CommercewebservicescommonsManager called. " + getTenant().getTenantID());
		}
	}
	...
	public void destroy()
	{
		if (LOG.isDebugEnabled())
		{
			LOG.debug("destroy() of CommercewebservicescommonsManager called, current tenant: " + getTenant().getTenantID());
		}
	}
