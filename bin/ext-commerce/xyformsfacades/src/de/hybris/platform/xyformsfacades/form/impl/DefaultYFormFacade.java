...
public class DefaultYFormFacade implements YFormFacade
{
			...
			throws YFormServiceException
	{
		...
		if (StringUtils.isEmpty(template))
		{
			...
			catch (final YFormServiceException e) // NOSONAR (exception is expected if yform definition is not found)
			{
		...
		try
		{
			...
			LOG.warn("You are trying to recreate a couterpart that already exists [" + applicationId + "][" + formId + "][" + version
					+ "]");
			return formDataId;
		}
		catch (final YFormServiceException e) // NOSONAR (exception is expected if yform data is not found)
			...
			throws YFormServiceException
	{
		...
		catch (final YFormServiceException e) // NOSONAR (exception is expected if yform data has not been found)
		{
	...
	@Override
	public String getFormDataContentTemplate(final String applicationId, final String formId) throws YFormServiceException
	{
