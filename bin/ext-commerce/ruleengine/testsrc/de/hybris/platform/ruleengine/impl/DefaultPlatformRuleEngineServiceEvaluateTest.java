...
public class DefaultPlatformRuleEngineServiceEvaluateTest
{
	...
	public void setUp()
	{
		...
		when(newKieContainer.newKieSessionsPool(anyInt())).thenReturn(kieContainerSessionsPool);
		when(oldKieContainer.newKieSessionsPool(anyInt())).thenReturn(kieContainerSessionsPool);
		when(kieContainerSessionsPool.newKieSession()).thenReturn(session);
		...
		when(kieSessionModel.getKieBase()).thenReturn(kieBaseModel);
		when(kieBaseModel.getKieModule()).thenReturn(droolsModule);
		...
		when(maxRuleExecutionsFunction.apply(ruleEvaluationContext)).thenReturn(-1);
	}
