...
public class B2bcommercefacadesManager extends GeneratedB2bcommercefacadesManager
{
	/** Edit the local|project.properties to change logging behavior (properties 'log4j.*'). */
	private static final Logger LOG = Logger.getLogger(B2bcommercefacadesManager.class.getName());
	 ...
	 */
	public static B2bcommercefacadesManager getInstance()
	{
		return (B2bcommercefacadesManager) Registry.getCurrentTenant().getJaloConnection().getExtensionManager()
				.getExtension(B2bcommercefacadesConstants.EXTENSIONNAME);
	}
	 ...
	 */
	public B2bcommercefacadesManager()
	{
		if (LOG.isDebugEnabled())
		{
			LOG.debug("constructor of B2bcommercefacadesManager called.");
		}
	}
	...
	public void init()
	{
		if (LOG.isDebugEnabled())
		{
			LOG.debug("init() of B2bcommercefacadesManager called. " + getTenant().getTenantID());
		}
	}
	...
	public void destroy()
	{
		if (LOG.isDebugEnabled())
		{
			LOG.debug("destroy() of B2bcommercefacadesManager called, current tenant: " + getTenant().getTenantID());
		}
	}
